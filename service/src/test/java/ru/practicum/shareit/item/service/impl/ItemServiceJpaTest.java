...
class ItemServiceJpaTest {
    ...
    void should_GetUserItems_ReturnListItems() {
        ...
        when(itemRepository.findByOwnerOrderByIdAsc(userId, PageRequest.of(from / size, size))).thenReturn(List.of(item));
        ...
        verify(itemRepository, times(1)).findByOwnerOrderByIdAsc(userId, PageRequest.of(from / size, size));
